...
- target ref: 기본 `HEAD`
- 선택 입력: PR 번호, issue 번호, 읽기 전용 GitHub metadata 실행 여부
- 선택 입력: report path, fail-on-risk 여부

**출력:**
- PR/issue metadata preview 또는 읽기 전용 결과
- 변경 파일 목록
- `git diff --check` 결과
...
PR 또는 issue 문맥을 같이 보고 싶으면 metadata preview를 붙입니다.

```bash
./scripts/review-workflow.sh --pr 123 --issue 42
```

사용자가 읽기 전용 GitHub 조회를 승인하면 `--run-gh`를 붙입니다.

```bash
./scripts/review-workflow.sh --pr 123 --issue 42 --run-gh
```
...
- `git diff --name-only`
- `git diff --check`
- PR/issue metadata preview 또는 읽기 전용 `gh pr view`, `gh issue view`
- diff risk scan
...
- PR/issue 문맥은 기본 preview이며, `--run-gh`는 읽기 전용 조회에만 사용합니다.
